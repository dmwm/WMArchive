...
app.visualizationRenderers.time = function(canvas, data, options) {
  ...
  if (is_stacked) {
    ...
    bar.append('g').selectAll('rect')
      ...
      .enter().append("circle")
        ...
        .attr('data-toggle', 'tooltip')
        .attr('title', function(d) {
          return d3.timeFormat(app.format_time_d3)(new Date(d.label.start_date)) + ": " + app.format_value(metric)(d.average);
        });
    ...
    chart.append("g")
      ...
      .call(d3.axisLeft(length).ticks(3).tickFormat(app.format_tick(metric)))
      .append("g")
        .attr("class", "label")
        .attr("transform", "translate(0," + 0 + ")")
        .append("text")
          ...
          .attr("y", -10)
          .attr("fill", "black")
          .text(app.format_ticks_label(metric))
